...
- `bun run test:quest` is the fastest laptop-side sanity check for Quest Bridge because it confirms the running bridge can answer locally before you put the headset on.
...
- The desktop UI now shows a recommended Quest URL, alternate LAN URLs, a desktop self-test URL, and the exact smoke command to copy before live headset testing.
